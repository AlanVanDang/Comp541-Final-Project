...
`ifdef SYNTHESIS
    parameter imem_init="imem.mem",    // version of program for synthesis, with pauses
`else
...
`endif
   ...
   wire [wordsize-1:0] seg_val; 
   memIO #(.w(wordsize), .dmem_size(dmem_size), .dmem_init(dmem_init), .Nchars(Nchars), .smem_size(smem_size), .smem_init(smem_init)
   ) memIO(
       .clock(clk12), .cpu_wr(mem_wr), .cpu_addr(mem_addr), .cpu_writedata(mem_writedata), .cpu_readdata(mem_readdata),
       .vga_addr(smem_addr), .vga_readdata(charcode), .keyb_char, .accel_val, .period, .lights(LED), .seg_val(seg_val));
   ...
   display8digit disp(.val(seg_val), .clk(clk100), .segments, .digitselect);
